...
const SettingLocation = ({isOpen, onEvent}) => {
  ...
  return (
    <div ref={modalPage} className={`main settingLocation modalPage ${isOpen?'open':''}`}>
      ...
      {/* <MainMap location={location} setLocation={setLocation} containerId={'locationMap'}/> */}
      <div className="locationBox">
        <div className="loactionBtn" onClick={fetchGeolocation}>
          <img src={btnHere} alt="현재위치"/>
        </div>
        <p className="location">{'서울 영등포구 여의도동 45'}</p>
        <InputText placeholder={'상세주소 입력 (예 : OO빌딩 앞, OO아파트 단지 내)'}/>
        <Button fullmode={true} text={'선택한 위치로 설정'}/>
      </div>
    </div>
  );
};
